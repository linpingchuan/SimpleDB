...
//  private Query owner;
   ...
   * the specified query, this method is just used so that the object can print
   * the SQL it represents.
   *
  ...
  */
   ...
   * fully qualified field (tablename.field or alias.field), or can be a unique
   * field name without a tablename qualifier. If it is an ambiguous name, it
   ...
   * field is an integer field, this should be a String representing an integer.
   *
   * @throws ParsingException If field is not in one of the tables added via
   * {@link #addScan} or if field is ambiguous (e.g., two tables contain a field
   * named field).
   */
   ...
   * unqualified unique field name. If the name is ambiguous or unknown, a
   * ParsingException will be thrown.
   *
  ...
  */
   ...
   * parameter is already qualified with a table name, simply returns name.
   *
   ...
   * each table used in the LogicalPlan. This should have one entry for each
   * table referenced by the plan, not one entry for each table alias (so a
   * table t aliases as t1 and t2 would have just one entry with key 't' in this
   * HashMap).
   *
  ...
  public DbIterator physicalPlan(TransactionId t, Map<String, TableStats> baseTableStats, boolean explain)
    ...
    while (joinIt.hasNext()) {
      ...
      if (!isSubqueryJoin) {
        ...
        // make sure anything that was equiv to lj.t2 (which we are just removed)
        // is marked as equiv to lj.t1 (which we are replacing lj.t2 with).
  ...
  public static void main(String argv[]) {
    ...
    // Create the tables, associate them with the data files and tell the
    // catalog about the schema the tables.
