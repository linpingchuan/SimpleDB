...
public class JoinOptimizer {
   ...
   * previous invocations of computeCostAndCardOfSubplan for this joinSet,
  ...
  private CostCard computeCostAndCardOfSubplan(
    ...
    if (news.isEmpty()) { // Base case, both are base relations.
      ...
      // Possible that we have not cached an answer, if subset includes a cross
      // product.
      if (prevBest == null) {
        return null;
      }
      ...
      if (doesJoin(prevBest, table1Alias)) { // j.t1 is in prevBest
        ...
        // Don't consider this plan if one of j.t1 or j.t2 isn't a table joined
        // in prevBest (cross product).
        return null;
      }
    }
  ...
  /**
   * Returns true if the specified table is in the list of joins, false
   * otherwise.
   */
  ...
  /**
   * Returns true if field is a primary key of the specified table, false
   * otherwise.
   * 
  ...
  /**
   * Returns true if a primary key field is joined by one of the joins in
   * joinlist.
   */
   ...
   * (where tables are indentified by their alias or name if no alias is given).
   */
